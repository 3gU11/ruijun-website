...
export async function createFaqConversation({ baseUrl, fetchImpl = fetch, context = {}, faqSessionId = '' }) {
  const body = { sourceChannel: 'repair_portal', context };
  if (SAFE_SESSION_ID.test(String(faqSessionId || '').trim())) body.faqSessionId = String(faqSessionId).trim();
  let response;
  try {
    response = await fetchImpl(`${requireBaseUrl(baseUrl)}/api/faq/v1/conversations`, {
      method: 'POST',
      credentials: 'include',
  ...
  if (!response.ok || !SAFE_SESSION_ID.test(String(payload?.faqSessionId || ''))) {
    throw requestError('FAQ_BFF_UNAVAILABLE', response.status);
  }
  return payload;
}
...
export async function streamFaqMessage({ baseUrl, fetchImpl = fetch, faqSessionId, message, context = {}, idempotencyKey, signal, onEvent }) {
  ...
  if (!question || question.length > 2_000 || key.length < 8 || key.length > 128) {
    throw requestError('INVALID_FAQ_MESSAGE');
  }
  let response;
  try {
    response = await fetchImpl(`${requireBaseUrl(baseUrl)}/api/faq/v1/messages`, {
      method: 'POST',
      credentials: 'include',
      signal,
    ...
    throw requestError(signal?.aborted ? 'FAQ_REQUEST_CANCELLED' : 'FAQ_BFF_UNAVAILABLE');
  }
  ...
  const emit = (frame) => {
    ...
    if (!json) return;
    try {
      onEvent?.({ event, data: JSON.parse(json) });
    } catch {
      // Malformed upstream frames cannot affect the repair form.
    }
  };
  try {
    while (true) {
...
export async function submitFaqFeedback({ baseUrl, fetchImpl = fetch, faqSessionId, requestId, helpful, reason = '', escalateToHuman = false }) {
  ...
  if (!SAFE_REQUEST_ID.test(normalizedRequestId) || typeof helpful !== 'boolean' || normalizedReason.length > 120) {
    throw requestError('INVALID_FAQ_FEEDBACK');
  }
  let response;
  try {
    response = await fetchImpl(`${requireBaseUrl(baseUrl)}/api/faq/v1/feedback`, {
      ...
      headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
      body: JSON.stringify({
        faqSessionId: requireSessionId(faqSessionId),
        requestId: normalizedRequestId,
        helpful,
        reason: normalizedReason,
        escalateToHuman: Boolean(escalateToHuman)
      })
    });
  } catch {
    throw requestError('FAQ_BFF_UNAVAILABLE');
