<template>
  <private-view title="产品技术审核">
    <div class="review-page">
      ...
      <div v-else-if="!models.length && !series.length && !parameters.length" class="empty">暂无可读取的产品草稿，或当前账号没有产品审核权限。</div>
      <template v-else>
        <section class="summary" aria-label="产品审核概览">
          ...
          <div><span>独立参数</span><strong>{{ parameters.length }}</strong></div>
        </section>
        ...
        <section class="panel" aria-label="独立产品参数审核项">
          <h3>独立产品参数</h3>
          <div v-if="!flaggedParameters.length" class="empty compact">当前可读取参数没有自动识别的缺项或冲突。</div>
          <table v-else>
            <thead><tr><th>型号</th><th>参数</th><th>值</th><th>审核提示</th><th>状态</th></tr></thead>
            <tbody><tr v-for="item in flaggedParameters" :key="item.id"><td>{{ item.model_code || '未填写' }}</td><td>{{ item.field_name || '未填写' }}</td><td>{{ item.value || '未填写' }}<span v-if="item.unit"> {{ item.unit }}</span></td><td>{{ item.issues.join('；') }}</td><td>{{ item.status || '未知' }}/{{ item.publication_state || '未知' }}</td></tr></tbody>
          </table>
        </section>
      </template>
    </div>
...
import { assessProductModel, assessProductParameter, assessProductSeries, sortProductRecords } from '../../../reports/product-review-rules.mjs';
...
const parameterFields = ['id', 'model_code', 'group_name', 'field_name', 'value', 'unit', 'source_url', 'source_document', 'import_evidence', 'status', 'publication_state'].join(',');
...
const flaggedParameters = computed(() => sortProductRecords(parameters.value.map((item) => ({ ...item, issues: assessProductParameter(item) })).filter((item) => item.issues.length), (item) => item.issues, (item) => `${item.model_code || ''}/${item.field_name || ''}`));

async function loadRecords() {
  loading.value = true;
  error.value = '';
  try {
    const [seriesResponse, modelsResponse, parametersResponse] = await Promise.all([
      ...
      api.get(`/items/product_parameters?fields=${encodeURIComponent(parameterFields)}&limit=-1`)
    ]);
    ...
    parameters.value = Array.isArray(parametersResponse.data?.data) ? parametersResponse.data.data : [];
  } catch (reason) {
    series.value = [];
    models.value = [];
    parameters.value = [];
