...
async function main() {
  ...
  try {
    ...
    assert.equal(asset.data.placement_key, 'service.hero.image');
    ...
    assert.equal(section.media.some((entry) => entry?.role === mediaRole), false, 'verification requires the empty service hero media baseline');
    ...
    assert.deepEqual(previewMedia, [{ role: mediaRole, media_asset_id: assetId, path: `/api/preview/media/${assetId}`, managed: true }]);

    const html = await (await fetch(`${websiteUrl}${location.pathname}${location.search}`, { headers: { cookie } })).text();
    assert.match(html, /data-cms-preview-placement-key="service\.hero\.image"/, 'rendered canvas did not declare the service hero placement');
    ...
    if (saved) {
      ...
      assert.deepEqual(sectionFor(current.data.sections).media.at(-1), { role: mediaRole, media_asset_id: assetId }, 'media changed during verification; refusing to overwrite it');
