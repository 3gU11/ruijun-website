...
const placementKey = 'about.hero.background';
const sourceDocument = 'website/public/assets/about-psd/hero-machine.jpg';
...
async function main() {
  ...
  try {
    ...
    section.media = [...existingMedia.filter((entry) => entry?.role !== 'background'), { role: 'background', media_asset_id: assetId }];
    ...
    assert.deepEqual(sectionFor(persisted.data.sections).media.at(-1), { role: 'background', media_asset_id: assetId });
    ...
    assert.ok(previewMedia.some((entry) => entry?.role === 'background' && entry?.media_asset_id === assetId && entry?.path === `/api/preview/media/${assetId}` && entry?.managed === true));
    const html = await (await fetch(`${websiteUrl}${location.pathname}${location.search}`, { headers: { cookie } })).text();
    assert.match(html, new RegExp(`/api/preview/media/${assetId}`)); assert.match(html, /data-cms-preview-placement-key="about\.hero\.background"/);
    console.log(JSON.stringify({ savedTo: `pages/${pageId}.sections[${sectionId}].media.background`, mediaAssetId: assetId, rendered: true }));
  } finally {
    if (saved) {
