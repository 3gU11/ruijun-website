...
const pageId = '3';
const sectionId = 'support-actions';
const marker = `visual-service-action-card-title-${Date.now()}`;
const presentation = {
...
function actionSection(sections) {
  const section = sections.find((item) => item?.id === sectionId);
  assert.ok(section, `pages/${pageId} must contain ${sectionId}`);
  assert.ok(Array.isArray(section.items) && section.items.length >= 2, `${sectionId} must contain at least two cards`);
  return section;
}
...
async function main() {
  ...
  try {
    ...
    const section = actionSection(sections);
    ...
    const savedSection = actionSection(persisted.data.sections);
    assert.equal(savedSection.items[0][field], marker, 'Directus did not persist the service action title');
    assertControlledPresentation(savedSection.items[0].field_presentation?.[field], 'Directus did not persist the controlled action title presentation');
    assert.deepEqual(savedSection.items[1], siblingSnapshot, 'Directus changed the adjacent service action card');
    ...
    const previewSection = actionSection(session.data.preview.sections);
    assert.equal(previewSection.items[0][field], marker, 'preview session omitted the service action title');
    assertControlledPresentation(previewSection.items[0].field_presentation?.[field], 'preview session omitted the action title presentation');
    assert.deepEqual(previewSection.items[1], siblingSnapshot, 'preview session changed the adjacent service action card');

    const html = await request(`${websiteUrl}${previewLocation.pathname}${previewLocation.search}`, { headers: { cookie } }, true);
    assert.ok(html.includes(marker), 'Nuxt preview did not render the saved service action title');
    assert.ok(html.includes('data-cms-preview-key="support-actions"'), 'Nuxt preview did not render the service action section');
    assert.ok(html.includes(`data-cms-preview-position-field-path="items.0.field_presentation.${field}"`), 'Nuxt preview did not render the action title presentation binding');
    assert.ok(html.includes('--cms-offset-x-desktop:3%'), 'Nuxt preview did not render the action title offset');
    assert.ok(html.includes('--cms-text-size-desktop:28px'), 'Nuxt preview did not render the action title size');

    summary = { savedTo: `pages/3.sections[support-actions].items[0].field_presentation.${field}`, previewTarget: 'pages/3/support-actions', previewTitle: marker };
  } finally {
    if (saved) {
      ...
      const section = actionSection(sections);
      const first = section.items[0];
      assert.equal(first[field], marker, 'service action title changed during verification; refusing to overwrite a newer editor value');
      assertControlledPresentation(first.field_presentation?.[field], 'service action title presentation changed during verification; refusing to overwrite a newer editor value');
      assert.deepEqual(section.items[1], siblingSnapshot, 'adjacent service action card changed during verification; refusing to overwrite');
      ...
      const restoredSection = actionSection(restored.data.sections);
      assert.equal(restoredSection.items[0][field], originalTitle, 'service action title was not restored');
      assert.deepEqual(restoredSection.items[0].field_presentation || null, originalPresentation, 'service action title presentation was not restored');
      assert.deepEqual(restoredSection.items[1], siblingSnapshot, 'adjacent service action card was not restored unchanged');
    }
  }
