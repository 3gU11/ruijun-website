...
async function main() {
  ...
  try {
    ...
    assert.equal(asset.data.usage_scope, 'brand'); assert.equal(asset.data.placement_key, 'about.timeline.background');
    ...
    assert.equal(section.media.some((entry) => entry?.role === mediaRole), false, 'verification requires the empty timeline background baseline');
    ...
    assert.deepEqual(sectionFor(session.data.preview.sections).media, [{ role: mediaRole, media_asset_id: assetId, path: `/api/preview/media/${assetId}`, managed: true }]);
    const html = await (await fetch(`${websiteUrl}${location.pathname}${location.search}`, { headers: { cookie } })).text();
    assert.match(html, new RegExp(`/api/preview/media/${assetId}`)); assert.match(html, /data-cms-preview-media-role="background"/);
