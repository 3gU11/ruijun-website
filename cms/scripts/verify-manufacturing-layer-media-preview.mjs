...
const pageId = '8';
const sectionId = 'precision-machining';
const layerRole = 'cnc-main';
const assetId = '56';
...
async function main() {
  ...
  try {
    ...
    assert.equal(asset.data.usage_scope, 'manufacturing');
    assert.equal(asset.data.placement_key, 'manufacturing.layer.image');
    assert.equal(asset.data.page_key, 'manufacturing');
    ...
    assert.equal(section.media.some((entry) => entry?.role === layerRole), false, 'verification requires the empty layer slot baseline');
    section.media.push({ role: layerRole, media_asset_id: assetId });
    ...
    assert.deepEqual(sectionFor(persisted.data.sections).media.at(-1), { role: layerRole, media_asset_id: assetId });
    ...
    assert.deepEqual(previewMedia, [{ role: layerRole, media_asset_id: assetId, path: `/api/preview/media/${assetId}`, managed: true }]);

    const html = await (await fetch(`${websiteUrl}${location.pathname}${location.search}`, { headers: { cookie } })).text();
    assert.match(html, new RegExp(`data-cms-preview-field-path=\\"media\\.0\\"`), 'rendered canvas did not expose the exact media field');
    assert.match(html, new RegExp(`/api/preview/media/${assetId}`), 'rendered canvas did not resolve the draft asset through the preview proxy');
    ...
    if (saved) {
      ...
      assert.deepEqual(sectionFor(current.data.sections).media.at(-1), { role: layerRole, media_asset_id: assetId }, 'media changed during verification; refusing to overwrite it');
      ...
      assert.deepEqual(restored.data.sections, originalSections, 'manufacturing page was not restored');
