...
async function main() {
  ...
  try {
    ...
    assert.equal(asset.data.usage_scope, 'brand'); assert.equal(asset.data.placement_key, 'about.gallery.image');
    assert.equal(asset.data.page_key, 'about'); assert.equal(asset.data.section_key, sectionId);
    ...
    assert.equal(section.media.some((entry) => entry?.role === 'factory-1'), false, 'verification requires the empty factory gallery baseline');
    section.media.push({ role: 'factory-1', media_asset_id: assetId });
    ...
    assert.deepEqual(sectionFor(persisted.data.sections).media.at(-1), { role: 'factory-1', media_asset_id: assetId });
    ...
    assert.deepEqual(sectionFor(session.data.preview.sections).media, [{ role: 'factory-1', media_asset_id: assetId, path: `/api/preview/media/${assetId}`, managed: true }]);
    const html = await (await fetch(`${websiteUrl}${location.pathname}${location.search}`, { headers: { cookie } })).text();
    assert.match(html, new RegExp(`/api/preview/media/${assetId}`)); assert.match(html, /data-cms-preview-media-role="gallery"/);
    console.log(JSON.stringify({ savedTo: `pages/${pageId}.sections[${sectionId}].media.0`, mediaAssetId: assetId, rendered: true }));
  } finally {
    if (saved) {
      const current = await requestJson(`${cmsUrl}/items/pages/${pageId}?fields=id,sections`, { headers });
      assert.deepEqual(sectionFor(current.data.sections).media.at(-1), { role: 'factory-1', media_asset_id: assetId });
