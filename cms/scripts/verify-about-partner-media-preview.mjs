...
async function main() {
  ...
  try {
    ...
    assert.equal(asset.data.placement_key, 'about.partner.image');
    ...
    assert.equal(section.media.some((entry) => entry?.role === 'partner-1'), false, 'verification requires an empty partner baseline');
    section.media.push({ role: 'partner-1', media_asset_id: assetId });
    ...
    assert.deepEqual(sectionFor(persisted.data.sections).media.at(-1), { role: 'partner-1', media_asset_id: assetId });
    const issued = await requestJson(`${cmsUrl}/content-preview-tokens/issue`, {
      method: 'POST', headers, body: JSON.stringify({ contentCollection: 'pages', contentItemId: pageId, ttlSeconds: 120 })
    });
    ...
    assert.deepEqual(sectionFor(session.data.preview.sections).media, [{ role: 'partner-1', media_asset_id: assetId, path: `/api/preview/media/${assetId}`, managed: true }]);
    ...
    assert.match(html, /data-cms-preview-placement-key="about\.partner\.image"/);
    ...
    if (saved) {
      ...
      assert.deepEqual(sectionFor(current.data.sections).media.at(-1), { role: 'partner-1', media_asset_id: assetId });
