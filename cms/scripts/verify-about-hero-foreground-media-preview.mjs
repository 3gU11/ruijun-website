...
const placementKey = 'about.hero.foreground';
const sourceDocument = 'website/public/assets/psd/reason-intro-machine.png';
...
async function main() {
  ...
  try {
    const [page, assetResult] = await Promise.all([
      requestJson(`${cmsUrl}/items/pages/${pageId}?fields=id,status,publication_state,sections`, { headers }),
      requestJson(`${cmsUrl}/items/media_assets?filter[source_document][_eq]=${encodeURIComponent(sourceDocument)}&fields=id,usage_scope,placement_key,page_key,section_key,status,publication_state&limit=1`, { headers })
    ]);
    const asset = Array.isArray(assetResult.data) ? assetResult.data[0] : null;
    assert.equal(page.data.status, 'draft'); assert.equal(page.data.publication_state, 'unpublished');
    assert.ok(asset?.id, `a governed draft asset is required for ${sourceDocument}`);
    const assetId = String(asset.id);
    assert.equal(asset.usage_scope, 'brand'); assert.equal(asset.placement_key, placementKey);
    assert.equal(asset.page_key, 'about'); assert.equal(asset.section_key, sectionId);
    assert.equal(asset.status, 'draft'); assert.equal(asset.publication_state, 'unpublished');

    originalSections = clone(page.data.sections || []);
    const sections = clone(originalSections); const section = sectionFor(sections);
    const existingMedia = Array.isArray(section.media) ? section.media : [];
    section.media = [...existingMedia.filter((entry) => entry?.role !== 'foreground'), { role: 'foreground', media_asset_id: assetId }];
    ...
    assert.deepEqual(sectionFor(persisted.data.sections).media.at(-1), { role: 'foreground', media_asset_id: assetId });
    ...
    assert.ok(previewMedia.some((entry) => entry?.role === 'foreground' && entry?.media_asset_id === assetId && entry?.path === `/api/preview/media/${assetId}` && entry?.managed === true));
    const html = await (await fetch(`${websiteUrl}${location.pathname}${location.search}`, { headers: { cookie } })).text();
    assert.match(html, new RegExp(`/api/preview/media/${assetId}`)); assert.match(html, /data-cms-preview-placement-key="about\.hero\.foreground"/);
    console.log(JSON.stringify({ savedTo: `pages/${pageId}.sections[${sectionId}].media.foreground`, mediaAssetId: assetId, rendered: true }));
  } finally {
    if (saved) {
