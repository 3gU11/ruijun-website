...
const pageId = '1';
const sectionId = 'advanced-manufacturing';
...
async function main() {
  ...
  try {
    ...
    assert.equal(asset.data.usage_scope, 'homepage'); assert.equal(asset.data.placement_key, 'home.reason.background');
    ...
    assert.equal(section.media.some((entry) => entry?.role === mediaRole), false, 'verification requires the empty reason background baseline');
    ...
    assert.match(html, new RegExp(`/api/preview/media/${assetId}`)); assert.match(html, /data-cms-preview-placement-key="home\.reason\.background"/);
