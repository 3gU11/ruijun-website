...
async function main() {
  ...
  try {
    ...
    assert.equal(asset.data.usage_scope, 'brand'); assert.equal(asset.data.placement_key, 'about.timeline.icon');
    ...
    assert.equal(section.media.some((entry) => entry?.role === mediaRole), false, 'verification requires the empty timeline icon baseline');
    ...
    assert.match(html, /data-cms-preview-placement-key="about\.timeline\.icon"/); assert.match(html, new RegExp(`/api/preview/media/${assetId}`));
