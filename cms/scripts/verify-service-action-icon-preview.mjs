...
async function requestJson(url, init = {}) {
  ...
  if (!response.ok) throw new Error(`${init.method || 'GET'} ${url} failed: ${response.status} ${text.slice(0, 240)}`);
...
async function main() {
  ...
  try {
    const [page, asset] = await Promise.all([
      requestJson(`${cmsUrl}/items/pages/${pageId}?fields=id,sections`, { headers }),
    ...
    assert.equal(asset.data.usage_scope, 'service');
    assert.equal(asset.data.placement_key, 'service.action.icon');
    assert.equal(asset.data.page_key, 'service');
    ...
    assert.equal(section.media.some((entry) => entry?.role === iconRole), false, 'verification requires the empty action icon baseline');
    section.media.push({ role: iconRole, media_asset_id: assetId });
    ...
    assert.deepEqual(sectionFor(persisted.data.sections).media.at(-1), { role: iconRole, media_asset_id: assetId });
    ...
    const resolvedMedia = sectionFor(session.data.preview.sections).media.filter((entry) => String(entry?.media_asset_id || '').trim());
    assert.deepEqual(resolvedMedia, [{ role: iconRole, media_asset_id: assetId, path: `/api/preview/media/${assetId}`, managed: true }]);

    const html = await (await fetch(`${websiteUrl}${location.pathname}${location.search}`, { headers: { cookie } })).text();
    assert.match(html, /data-cms-preview-field-path=\"media\.0\"/, 'rendered canvas did not expose the service icon field');
    assert.match(html, /data-cms-preview-media-slot=\"action-1\"/, 'rendered canvas did not identify the icon slot');
    assert.match(html, /data-cms-preview-placement-key=\"service\.action\.icon\"/, 'rendered canvas did not identify the governed placement');
    assert.match(html, new RegExp(`/api/preview/media/${assetId}`), 'rendered canvas did not resolve the draft icon through the preview proxy');
    ...
    if (saved) {
      ...
      assert.deepEqual(sectionFor(current.data.sections).media.find((entry) => entry?.role === iconRole), { role: iconRole, media_asset_id: assetId }, 'media changed during verification; refusing to overwrite it');
      ...
      assert.deepEqual(restored.data.sections, originalSections, 'service page was not restored');
