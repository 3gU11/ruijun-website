...
<template>
  <main class="home-page">
    ...
    <section id="home" class="hero lifecycle-panel" :class="{ 'hero-ended': heroState.stageVisible }" data-panel-label="首页" aria-labelledby="hero-title">
      <img v-if="videoFailed" class="hero-end-frame" src="/assets/home-intro-end-aligned.png?v=3" alt="">
      <video v-if="heroState.videoVisible" ref="heroVideo" class="hero-video" :class="{ 'is-ended': videoCompleted }" autoplay muted playsinline @ended="finishHeroVideo" @error="handleHeroVideoError"><source src="/assets/home-intro.mp4" type="video/mp4"></video>
      <img class="hero-static-machine" src="/assets/psd/hero-machine.png" alt="瑞钧中走丝线切割机床">
      <h1 id="hero-title" class="sr-only">{{ heading }}</h1>
      <a class="hero-ended-link" :class="{ 'is-visible': showHeroNextButton }" href="#reasons">了解更多 <span aria-hidden="true">→</span></a>
    </section>
    ...
    <section id="products" class="products lifecycle-panel" data-panel-label="产品中心" aria-labelledby="products-title">
      <div class="product-shell">
        <div class="product-heading"><p class="product-heading-en">Our product</p><h2 id="products-title" class="product-heading-cn"><span>我们的</span><em>产品</em></h2></div>
    ...
    <section id="contact" class="contact-panel lifecycle-panel" data-panel-label="联系瑞钧"><div class="contact-hero"><p>RUIJUN MEDIUM SPEED WIRE EDM</p><h2>让下一台设备<br>匹配你的加工任务</h2><NuxtLink to="/contact">获取选型建议 <span aria-hidden="true">→</span></NuxtLink></div><SiteFooter variant="full" home-psd /></section>
  </main>
</template>
...
@media(min-width:901px){
  ...
  .hero.lifecycle-panel>.hero-static-machine{top:33.6svh;left:16.75%;width:33.2vw;max-width:none;visibility:hidden!important;opacity:0!important;transition:opacity .52s cubic-bezier(.22,.8,.24,1)}
  .hero.lifecycle-panel.hero-ended>.hero-static-machine{visibility:hidden!important;opacity:0!important}
  ...
  .performance-content h2{margin:0;font-size:clamp(36px,3.55vw,68px);line-height:1.2;font-weight:500;white-space:nowrap;transform:none}
  ...
  .history-copy{top:24%;right:auto;left:13.67%;width:30%;padding:0}
