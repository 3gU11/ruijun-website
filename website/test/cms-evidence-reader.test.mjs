...
test('qualification assets retain source indexes when unsafe entries are filtered', async () => {
  const reader = createCmsEvidenceReader({
    qualificationsEndpoint: 'https://cms.example.test/items/qualifications',
    fetchImpl: async () => Response.json({ data: [{
      id: 7, source_key: 'certificate-07', type: 'certificate', name: 'Certificate',
      authorization_status: 'approved', ...published,
      assets: [{ path: 'javascript:alert(1)' }, { path: '/assets/certificate.jpg' }]
    }] })
  });
  const result = await reader.listQualifications();
  assert.equal(result.data[0].assets[0].sourceIndex, 1);
});
...
test('Nuxt evidence reader exposes only published, effective, approved and safe public evidence', async () => {
  ...
  const reader = createCmsEvidenceReader({
    ...
    fetchImpl: async (url, options) => {
      ...
      if (request.pathname.endsWith('/milestones')) return Response.json({ data: [
        { id: 'milestone-1997', source_key: 'timeline-1997', year: 1997, event: 'Founded', evidence: 'Reviewed source', sort_order: 1, ...published },
      ...
      if (request.pathname.endsWith('/qualifications')) return Response.json({ data: [
        { id: 'qualification-01', source_key: 'certificate-01', type: 'certificate', name: 'Quality certificate', assets: [{ path: '/assets/certificates/quality.jpg', alt: 'Quality certificate' }, { path: 'javascript:alert(1)', alt: 'Unsafe' }], authorization_status: 'approved', sort_order: 1, ...published },
      ...
      return Response.json({ data: [
        { id: 'evidence-precision', source_key: 'precision', process: 'precision-machining', description: 'Reviewed process', media: [{ path: 'https://cdn.example.test/factory.jpg', alt: 'Factory' }], inspection_evidence: 'Report', sort_order: 1, ...published },
  ...
  assert.deepEqual(await reader.listMilestones(), {
    data: [{ id: 'milestone-1997', source_key: 'timeline-1997', year: 1997, event: 'Founded', evidence: 'Reviewed source', sort_order: 1 }], cache: 'fresh', source: 'cms'
  });
  assert.deepEqual(await reader.listQualifications(), {
    data: [{ id: 'qualification-01', source_key: 'certificate-01', type: 'certificate', name: 'Quality certificate', assets: [{ path: '/assets/certificates/quality.jpg', sourceIndex: 0, alt: 'Quality certificate' }], sort_order: 1 }], cache: 'fresh', source: 'cms'
  });
  assert.deepEqual(await reader.listManufacturingEvidence(), {
    data: [{ id: 'evidence-precision', source_key: 'precision', process: 'precision-machining', description: 'Reviewed process', media: [{ path: 'https://cdn.example.test/factory.jpg', alt: 'Factory' }], inspection_evidence: 'Report', sort_order: 1 }], cache: 'fresh', source: 'cms'
  });
  ...
  assert.match(requests[0].request.searchParams.get('fields'), /(?:^|,)id(?:,|$)/);
  assert.match(requests[1].request.searchParams.get('fields'), /authorization_status/);
  assert.match(requests[1].request.searchParams.get('fields'), /(?:^|,)id(?:,|$)/);
  assert.match(requests[2].request.searchParams.get('fields'), /(?:^|,)id(?:,|$)/);
});
...
test('Nuxt evidence reader resolves published asset IDs for qualifications and manufacturing evidence', async () => {
  const reader = createCmsEvidenceReader({
    ...
    fetchImpl: async (url) => {
      ...
      if (request.pathname.endsWith('/qualifications')) return Response.json({ data: [
        { id: 'qualification-01', source_key: 'certificate-01', type: 'certificate', name: 'Quality certificate', assets: [{ media_asset_id: 9 }], authorization_status: 'approved', sort_order: 1, ...published }
      ] });
      return Response.json({ data: [
        { id: 42, source_key: 'precision', process: 'precision-machining', description: 'Reviewed process', media: [{ media_asset_id: 9 }], inspection_evidence: 'Report', sort_order: 1, ...published }
      ] });
    }
  });

  const qualification = (await reader.listQualifications()).data[0];
  assert.equal(qualification?.id, 'qualification-01');
  assert.deepEqual(qualification?.assets, [{ path: 'https://cms.example.test/assets/certificate-file', sourceIndex: 0, managed: true, alt: '审核证书', mediaType: 'image' }]);
  assert.equal((await reader.listManufacturingEvidence()).data[0]?.id, '42');
  assert.deepEqual((await reader.listManufacturingEvidence()).data[0]?.media, [{ path: 'https://cms.example.test/assets/certificate-file', managed: true, alt: '审核证书', mediaType: 'image' }]);
});
