...
export function useCmsServiceContentReader(config: ReturnType<typeof useRuntimeConfig>) {
  ...
  const key = `${resourcesEndpoint}:${locationsEndpoint}:${Boolean(accessToken)}:${cacheTtlMs}`;
  let reader = readers.get(key);
  if (!reader) {
    reader = createCmsServiceContentReader({ resourcesEndpoint, locationsEndpoint, accessToken, cacheTtlMs });
